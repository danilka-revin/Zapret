...
12 акцентов, 8 пресетов стекла, стратегии и сеть в один клик, профили,
замеры скорости, системный трей.
...
| Светлая тема | Все стратегии |
| --- | --- |
| ![Светлая тема](assets/screenshots/main-light.png) | ![Стратегии](assets/screenshots/strategies.png) |

| Подбор под конкретный сайт | Диагностика |
| --- | --- |
| ![Подбор под сайт](assets/screenshots/targets.png) | ![Диагностика](assets/screenshots/diagnostics.png) |

| Сеть и файрвол | Журнал |
| --- | --- |
| ![Сеть](assets/screenshots/network.png) | ![Журнал](assets/screenshots/journal.png) |

Тёмная и светлая темы переключаются одной кнопкой **☀ / ☾** в шапке и действуют
на всё окно: палитра приложения (поля ввода, подсказки, меню, таблицы, полосы
прокрутки) перекрашивается вместе с интерфейсом, светлых «заплаток» не остаётся.
...
### Экран
- Большая кнопка включения с прогрессом, пульсацией, свечением и подписью
  «что сейчас проверяется». Длинные операции прерываются кнопкой «Остановить».
- Живая проверка YouTube, Discord, Telegram: задержка, статус, повторный запуск,
  замер скорости.
- График скорости: приём и отдача двумя линиями, пики, суммарный трафик,
  кнопки пинга и замера скорости.
- Быстрая смена стратегии и профили конфигурации прямо на главном экране.
- Журнал всех операций с фильтрами, поиском, копированием и выгрузкой в файл.
- Статус-бар: версия, бэкенд, стратегия, аптайм.

### Кастомизация (панель «Оформление»)
- Тема: **светлая / тёмная / системная**.
- Акцент: 12 пресетов + любой цвет через системный выбор цвета.
...
- 8 готовых пресетов в один клик + шаринг тем через буфер обмена.
- Всё сохраняется в `config.json`.

### Панели (шторки справа)
- **Стратегии** — все стратегии списком: порты, фильтры, превью, применение.
- **Сеть и файрвол** — интерфейс, бэкенд, GameFilter, версии, пинг/DNS/скорость.
- **О программе** — версия, чейнджлог с GitHub, ссылки.
- **Диагностика** — правила файрвола, отчёты, пинг, скорость, автоподбор.
- **Журнал** — поиск, фильтры по уровням, выгрузка в файл.
...
- Профили конфигурации: «Дом», «Игры», «Работа» — переключение одним нажатием.
- Автозапуск системы и при входе, автовосстановление, трей, уведомления,
  период проверок, история событий.
...
| `Ctrl+B` | Все стратегии |
| `Ctrl+E` | Сеть и файрвол |
...
python3 run.py strategy list|set|test|preview     # стратегии без интерфейса
python3 run.py speedtest                # замер скорости соединения
python3 run.py ping [host]              # пинг узла
...
python3 run.py history [--clear]        # история событий
...
```bash
python3 -m pytest tests -q                    # 155 модульных тестов: ядро, тема, пресеты, подбор,
                                              # ярлыки, сессия, обновление, починка, пинг,
                                              # скорость, стратегии, версии, чейнджлог
