...
Команда проверяет всё по очереди (Python, Qt и плагин `xcb`, nfqws, стратегии,
файрвол, права, службу, трей) и **пишет готовую команду установки** для вашего
дистрибутива. Девять из десяти «кнопка не работает» лечатся этим выводом.
...
### 🎯 Подбор под конкретный сайт
Бывает, что YouTube летает, а один сайт не открывается. Тогда нужен не общий
перебор, а стратегия именно под этот сайт — кнопка **«Подбор под сайт»**
(или `Ctrl+T`).

- вводите домен (`rutracker.org`, ссылку, несколько сайтов через запятую) или
  жмёте готовую группу: YouTube, Discord, Telegram, Нейросети, Соцсети, Медиа;
...
- недавние запросы сохраняются в истории и запускаются одним кликом.
...
```bash
python3 -m pytest tests -q                    # 39 модульных тестов ядра, темы и подбора
python3 tools/smoke_test.py                   # 50 проверок: клики по всем кнопкам
...
2. **Окно вообще не открывается.** Не установлен PySide6 или библиотеки Qt:
   ...
   для вашей системы. На Wayland помогает `QT_QPA_PLATFORM=wayland python3 run.py gui`.
...
6. **Всё равно не ясно.** Откройте «Диагностику» (видно состояние nfqws, правил,
   прав, бэкенда и интерфейса) и «Журнал» — последняя строка обычно объясняет
   причину словами.
