...
```bash
python3 -m pytest tests -q                    # 112 модульных теста: ядро, тема, пресеты, подбор,
                                              # ярлыки, сессия, обновление, починка
python3 tools/smoke_test.py                   # 67 проверок: клики по всем кнопкам
...
6. **Окно открылось, но ни одна кнопка не нажимается и не прокручивается.**
   Так выглядит 2.1.0: пустой контейнер перекрывал содержимое окна и забирал все
   события мыши. Обновитесь (`bash install.sh update` или «Обновить и перезапустить») —
   там фон и прокрутка живут внутри `central widget`, а после показа окна приложение
   само пишет в журнал, если содержимое кем-то перекрыто. Проверить можно так:
   ```bash
   QT_QPA_PLATFORM=offscreen python3 tools/smoke_test.py | grep -A6 "Клик и колесо"
   ```
